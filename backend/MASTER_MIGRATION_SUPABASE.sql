...
drop index if exists uq_one_active_booking_per_farmer;
...
-- Append-only admin rate history used by the farmer price trend chart.
create table if not exists public.crop_rate_history (
...
create index if not exists idx_crop_rate_history on public.crop_rate_history(crop, recorded_at);

-- Seed the initial Admin rates into history once so the farmer price chart
-- has real values immediately after the migration.
insert into public.crop_rate_history(crop, quality_grade, rate_per_kg, effective_from)
select r.crop, r.quality_grade, r.rate_per_kg, r.effective_from
from public.crop_rates r
where not exists (select 1 from public.crop_rate_history h where h.crop=r.crop and h.quality_grade=r.quality_grade);
